...
DJCStarlight.init = function() {
    ...
    engine.connectControl("[Channel3]","VuMeterL","DJCStarlight.baseLEDUpdate");
    engine.connectControl("[Channel4]","VuMeterR","DJCStarlight.baseLEDUpdate");
...
DJCStarlight._scrollLibrary = function(group, value) {
    var direction = DJCStarlight._convertWheelRotation(value);
...
// Jog wheel edge with shift (rotating using the edge)
DJCStarlight.bendWheelShift = function(channel, control, value, status, group) {
    DJCStarlight._scrollLibrary(group, value);
};
...
DJCStarlight.shiftButton = function(channel, control, value, status, group) {
    if (value >= 0x40) {
        ...
        var cueChan1LedValue =
            engine.getValue('[Channel3]', 'pfl') ? 0x7F : 0x00;
        midi.sendShortMsg(0x91, 0x0C, cueChan1LedValue);
        var cueChan2LedValue =
            engine.getValue('[Channel4]', 'pfl') ? 0x7F : 0x00;
